...
public class Tank implements Runnable{
    ...
    //增加成员，可以得到其他坦克的坐标（防碰撞）
    private static Vector<Tank> allTanks = new Vector<>();

    //坦克移动 上
    public void moveUp() {
        //先改变坦克的方向
        setDirect(0);
        if (y > 0 && !isTouchOtherTank()) {
            //再修改坦克的坐标，使其移动
            setY(getY() - speed);
        }
    }
        ...
        switch (direct) {
            case 0://上
                ...
                for (int i = 0; i < allTanks.size(); i++) {
                    ...
                    if (otherTank != this) {
                        //判断敌人的方向:上下
                        if (otherTank.direct == 0 || otherTank.direct == 1) {
                            //左上角范围
                            if (x >= otherTank.x
                                    && x <= otherTank.x + 40
                                    && y >= otherTank.y
    ...
    public static void setAllTanks(Vector<Tank> otherTanks) {
        allTanks = otherTanks;
    }
}
